<template>
  ...
  <div class="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 p-6">
    <div class="max-w-4xl mx-auto">
      <!-- Page Header -->
      <div
        class="bg-white rounded-xl shadow-lg border border-slate-200 p-6 mb-6"
      >
      ...
      <div class="bg-white rounded-xl shadow-lg border border-slate-200 p-8">
        <form @submit.prevent="submitForm" class="space-y-8">
          ...
          <div>
            <label
              class="block text-slate-700 text-sm font-semibold mb-3"
              for="title"
            >
              <Icon
                name="fluent:text-20-filled"
...
export default defineComponent({
  ...
  methods: {
    async init() {
      ...
      try {
        const workSettingRecord = await WorkSettingRepository.getById(
          parseInt(workSettingId),
          parseInt(userId)
        );
